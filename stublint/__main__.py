...
def lint_file(path: Path, strict: bool=False) -> bool:
    try:
    ...
    visitor = LintVisitor(path, strict=strict)
    visitor.visit(tree)
    return not visitor.saw_error


def run(args: Iterable[str], strict: bool=False) -> bool:
    return all([lint_file(path, strict=strict) for path in extract_files(args)])


if __name__ == '__main__':
    parser = argparse.ArgumentParser('stublint')
    parser.add_argument('--strict', help='Run in strict mode (disallows any missing type annotations)',
                        action='store_true')
    parser.add_argument('dirs_or_files', nargs='+', help='Files to run stublint on')
    args = parser.parse_args()
    try:
        sys.exit(0 if run(args.dirs_or_files, strict=args.strict) else 0)
